...
import { recalculateCalibration } from '../models/calibration.js';
...
export async function evaluatePredictions() {
  ...
  recalculateAccuracy();
  for (const h of ['1d', '5d', '30d']) recalculateCalibration(h);
...
function recalculateAccuracy() {
  ...
  for (const horizon of ['1d', '5d', '30d']) {
    ...
    db.prepare(`
      UPDATE accuracy_metrics SET total = ?, correct = ?, accuracy = ?, updated_at = CURRENT_TIMESTAMP
      WHERE horizon = ?
