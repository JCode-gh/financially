    ...
    <template v-else>
      ...
      <div class="grid grid-cols-3 gap-2 mb-2">
        <div v-for="h in horizons" :key="h.id" class="card-sm p-2 text-center">
          ...
          <div class="text-[10px] text-gray-600 font-mono">{{ $t('accuracy.live', { correct: h.liveCorrect, total: h.liveTotal }) }}</div>
          <div class="mt-1 pt-1 border-t border-surface-300/50 text-[10px] font-mono flex items-center justify-center gap-1">
            <span class="text-gray-500">{{ $t('accuracy.backtest') }}</span>
            <span :class="accuracyColor(h.bt)">{{ h.btTotal > 0 ? (h.bt * 100).toFixed(1) + '%' : '—' }}</span>
          </div>
          <div class="text-[10px] text-gray-600 font-mono">{{ $t('accuracy.calls', { n: h.btTotal }) }}</div>
        </div>
      </div>

      <div class="flex items-center gap-2 text-[10px] font-mono text-gray-600 mb-2">
        <span>{{ $t('accuracy.iteration', { n: accuracy.modelIteration || 0 }) }}</span>
        <span v-if="trainedAt" class="ml-auto">{{ $t('accuracy.trained', { ago: trainedAt }) }}</span>
      </div>

      <!-- Signed weight distribution -->
      <div class="text-xs text-gray-500 font-mono uppercase tracking-wider mb-1">{{ $t('accuracy.weights') }}</div>
      <div class="text-[10px] text-gray-600 font-mono mb-1.5">{{ $t('accuracy.weightsHint') }}</div>
      <div class="space-y-1">
...
const horizons = computed(() => {
  ...
  return accuracy.value.horizons.map(h => ({
    id: h.horizon,
    label: h.horizon === '1d' ? t('accuracy.day1') : h.horizon === '5d' ? t('accuracy.day5') : t('accuracy.day30'),
    live: h.accuracy || 0,
    liveTotal: h.total || 0,
...
const trainedAt = computed(() => {
  ...
  return isNaN(d.getTime()) ? null : d.toLocaleDateString(intlLocale(readLocale()), { month: 'short', day: 'numeric' });
});
